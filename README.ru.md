...
`topic_config.json` с настройками по умолчанию. Если topic существовал до запуска
бота, отправьте в него обычное сообщение от разрешенного пользователя: бот
зарегистрирует `message_thread_id` при первом сообщении. Topic сразу принимает
сообщения, но работает на defaults, пока вы его не настроите.
...
config. Чтобы узнать topic ID, запустите бота, отправьте сообщение в topic,
затем откройте сгенерированный `topic_config.json` и отредактируйте новую запись.
Для старого topic без доступного события создания временное имя будет иметь вид
`Topic <message_thread_id>`; его можно заменить вручную.
...
- `/research`: разрешить Codex живой веб-поиск только для следующей задачи.
  ...
  его. Во всех остальных запусках Codex поиск явно выключен. Если Codex
